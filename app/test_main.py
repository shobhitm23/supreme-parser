...
def test_empty_file():
    ...
    response = client.post("/", json={"filename": filename, "from": from_time, "to": to_time})
...
def test_invalid_file_format():
    ...
    assert response.json() == {"error": "Invalid file format. File must be of .txt format"}
...
def test_invalid_timestamps_format():
    ...
    "Invalid timestamp format. Timestamp must be in iso8601 UTC Format - YYYY-MM-DDThh:mm:ssZ"}
...
def test_file_not_found():
    ...
    response = client.post("/", json={"filename": "non_existent_file.txt", "from": "2022-10-22T00:00:00Z", "to": "2022-10-23T00:00:00Z"})
    
    assert response.status_code == 404
    assert response.json() == {"error": "File Not Found error: '/app/test-files/non_existent_file.txt'"}
